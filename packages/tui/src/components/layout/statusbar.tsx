...
function ConnectionIndicator() {
  ...
  const display = (
    {
      connected:    { text: '█', fg: SemanticColors.success.toHex(), label: 'Connected' },
      polling:      { text: '▌', fg: SemanticColors.info.toHex(),    label: 'Polling' },
      reconnecting: { text: '▌', fg: SemanticColors.warning.toHex(), label: 'Reconnecting' },
      offline:      { text: '░', fg: SemanticColors.error.toHex(),   label: 'Offline' },
    } as const
  )[connectionStatus] ?? { text: '░', fg: SemanticColors.error.toHex(), label: 'Offline' }
...
function WorkerCount() {
  ...
  return (
    <box flexDirection="row" gap={1}>
      ...
      <text fg={online === total ? SemanticColors.success.toHex() : SemanticColors.warning.toHex()}>
        {online}/{total}
      </text>
...
function PnLDisplay() {
  ...
  return (
    <box flexDirection="row" gap={1}>
      ...
      <text fg={isPositive ? SemanticColors.success.toHex() : SemanticColors.error.toHex()} bold>
        {formatted}
      </text>
    </box>
  )
