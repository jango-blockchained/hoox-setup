import { print_success, print_error, print_warning, print_info } from "../utils.js";
import {
...
export async function setupValidate(verbose: boolean = false, fix: boolean = false): Promise<void> {
  print_info("Running pre-flight validation...\n");

  const results: Array<{ name: string; success: boolean; errors: string[]; warnings: string[] }> = [];

  const depsResult = await validateDependencies();
  results.push({ name: "Dependencies", ...depsResult });
  console.log(`${depsResult.success ? "✅" : "❌"} Dependencies: ${depsResult.errors.length ? depsResult.errors.join(", ") : "OK"}`);
  ...
  console.log(`${authResult.success ? "✅" : "❌"} Auth: ${authResult.errors.length ? authResult.errors.join(", ") : "OK"}`);
  ...
  console.log(`${configResult.success ? "✅" : "❌"} Config: ${configResult.errors.length ? configResult.errors.join(", ") : "OK"}`);
  ...
  console.log(`${workersResult.success ? "✅" : "❌"} Workers: ${workersResult.errors.length ? workersResult.errors.join(", ") : "OK"}`);

  const resourcesResult = await validateResources(config);
  results.push({ name: "Resources", ...resourcesResult });
  console.log(`${resourcesResult.success ? "✅" : "❌"} Resources: ${resourcesResult.errors.length ? resourcesResult.errors.join(", ") : "OK"}`);
...
export async function setupRepair(): Promise<void> {
  ...
  const config = await loadConfig().catch(() => null);
  if (!config) {
    print_error("No configuration found. Running setup:validate --fix first might help.");
    return;
  }
  ...
  fixDependencies().then(async () => {
    ...
    if (depsResult.success) {
      ...
      print_warning(`Could not repair all dependencies: ${depsResult.errors.join(", ")}`);
    }
  });
  ...
  if (repairResult.success) {
    ...
    print_error(`Resource repair needs manual attention: ${repairResult.errors.join(", ")}`);
  }
...
export async function setupExport(): Promise<void> {
  ...
  const exportConfig = {
    global: {
      cloudflare_api_token: config.global.cloudflare_api_token ? "<TOKENS_REMOVED>" : "",
      cloudflare_account_id: config.global.cloudflare_account_id,
      cloudflare_secret_store_id: config.global.cloudflare_secret_store_id ? "<SECRET_STORE_ID>" : "",
