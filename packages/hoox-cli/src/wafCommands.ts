...
export async function setupWAF(config: Config): Promise<void> {
  ...
  const zoneId = await rl.question(
    blue(
      "Enter your Cloudflare Zone ID (required for WAF rules, cannot use workers.dev): "
    )
  );
  if (!zoneId) {
  ...
  try {
    ...
    if (!getRulesetsResponse.ok) {
      throw new Error(
        `Failed to fetch rulesets: ${await getRulesetsResponse.text()}`
      );
    }

    const rulesetsData = asRulesetList(await getRulesetsResponse.json());
    const customRuleset = rulesetsData.result.find(
      (r) => r.phase === "http_request_firewall_custom"
    );

    const rulesetId = customRuleset?.id;

    // The expression for the hoox worker path
    // Assuming hoox is accessible via a custom domain route, e.g., hoox.yourdomain.com
    const workerHostname = await rl.question(
      blue(
        "Enter the hostname your hoox worker is routed to (e.g., hoox.example.com): "
      )
    );
    ...
    if (!rulesetId) {
      ...
      if (!createResponse.ok) {
        throw new Error(
          `Failed to create ruleset: ${await createResponse.text()}`
        );
      }
      ...
      const existingRuleIndex = existingRules.findIndex(
        (r) => r.description === "Hoox - TradingView IP Allowlist"
      );
      ...
      if (!updateResponse.ok) {
        throw new Error(
          `Failed to update ruleset: ${await updateResponse.text()}`
        );
      }
    ...
    const rateLimitRuleset = rulesetsData.result.find(
      (r) => r.phase === "http_ratelimit"
    );
    const rlRulesetId = rateLimitRuleset?.id;
    ...
    if (!rlRulesetId) {
      ...
      if (!createRlResponse.ok) {
        throw new Error(
          `Failed to create rate limit ruleset: ${await createRlResponse.text()}`
        );
      }
      print_success("Created rate limit rule for Hoox webhooks.");
    } else {
      console.log(
        dim(`Updating existing rate limit ruleset (${rlRulesetId})...`)
      );
      const getRlRulesetResponse = await fetch(
        `${WAF_RULESET_URL}/${rlRulesetId}`,
        {
          headers: { Authorization: `Bearer ${apiToken}` },
        }
      );
      ...
      const existingRlRuleIndex = existingRlRules.findIndex(
        (r) => r.description === "Hoox - Webhook Rate Limit (10/min)"
      );
      ...
      const updateRlResponse = await fetch(
        `${WAF_RULESET_URL}/${rlRulesetId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!updateRlResponse.ok) {
        throw new Error(
          `Failed to update rate limit ruleset: ${await updateRlResponse.text()}`
        );
      }
    ...
    console.log(
      yellow(
        "Note: WAF rules are active on the specified hostname. Ensure your worker is routed via a Custom Domain."
      )
    );
  } catch (error: unknown) {
