...
export async function loadConfig(): Promise<Config> {
  ...
  if (await userFile.exists()) {
    ...
    console.warn(
      `Warning: ${getWorkersJsoncPath()} not found. Using example configuration as base.`
    );
  }

  if (!(await exampleFile.exists())) {
    throw new Error(
      `Example config file ${getWorkersExamplePath()} not found.`
    );
  }
  ...
  if (userConfigContent) {
    try {
      ...
      console.error(
        `Failed to parse ${getWorkersJsoncPath()} payload:`,
        redactForLogs({ workers: userConfigContent })
      );
  ...
  try {
    ...
    if (typeof parsedExample !== "object" || parsedExample === null) {
      throw new Error(
        `Example config ${getWorkersExamplePath()} did not parse to an object.`
      );
    }
    if (!("global" in parsedExample) || !("workers" in parsedExample)) {
      throw new Error(
        `Example config ${getWorkersExamplePath()} missing required 'global' or 'workers' sections.`
      );
    }
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error(
      `Failed to parse ${getWorkersExamplePath()} payload:`,
      redactForLogs({ workersExample: exampleConfigContent })
    );
  ...
  for (const key of requiredGlobalKeys) {
    if (!mergedConfig.global[key]) {
      console.error(
        `Error: Missing required global configuration key "${key}" in ${getWorkersJsoncPath()} (or example). Please define it.`
      );
      missingKeys = true;
    }
  }
...
export async function saveConfig(config: Config): Promise<void> {
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error(
      `Error saving configuration to ${getWorkersJsoncPath()}:`,
      errorMsg
    );
...
export async function loadPagesConfig(): Promise<{
  global: GlobalConfig;
  pages: Record<string, PagesConfig>;
}> {
  ...
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.warn(
      `Failed to parse pages.jsonc: ${errorMsg}. Returning empty config.`
    );
    console.warn(
      `Failed pages.jsonc payload:`,
      redactForLogs({ pages: await pagesFile.text().catch(() => "") })
    );
    return { global: {} as GlobalConfig, pages: {} };
  }
}

export async function savePagesConfig(pagesConfig: {
  global: GlobalConfig;
  pages: Record<string, PagesConfig>;
}): Promise<void> {
  try {
    ...
    console.log(
      `Pages configuration saved successfully to ${getPagesJsoncPath()}`
    );
  } catch (error: unknown) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    console.error(
      `Error saving pages configuration to ${getPagesJsoncPath()}:`,
      errorMsg
    );
