            ...
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="item-1">
                ...
                <AccordionContent className="text-xs text-muted-foreground pt-4 px-3 leading-relaxed">
                  Ensure you have deployed all the required worker components to
                  your Cloudflare account. Run{" "}
                  <code className="font-mono bg-secondary px-1.5 py-0.5 rounded text-[10px]">
                    bun run scripts/manage.ts workers deploy
                  </code>{" "}
                  in your terminal. This will provision the necessary edge
                  infrastructure.
                </AccordionContent>
              </AccordionItem>
              <AccordionItem value="item-2">
                ...
                <AccordionContent className="text-xs text-muted-foreground pt-4 px-3 leading-relaxed">
                  Go to your exchange (e.g., Binance, MEXC) and generate API
                  Keys with trading permissions. Do <strong>not</strong> enable
                  withdrawal permissions. Use the CLI commands listed below to
                  securely store these keys in the Cloudflare Secret Store.
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem value="item-5">
                <AccordionTrigger className="text-sm font-medium hover:no-underline hover:bg-secondary/20 px-3 rounded-md transition-colors">
                  5. Manage the Kill Switch
                </AccordionTrigger>
                <AccordionContent className="text-xs text-muted-foreground pt-4 px-3 leading-relaxed">
                  If you need to halt all trading activities immediately, head
                  over to the <strong>Settings</strong> page to toggle the
                  Global Kill Switch, or run{" "}
                  <code className="font-mono bg-secondary px-1.5 py-0.5 rounded text-[10px]">
                    bun run scripts/manage.ts kill-switch --enable
                  </code>{" "}
                  via CLI.
                </AccordionContent>
              </AccordionItem>
                ...
                <div className="p-4 bg-muted/20 border-t border-border">
                  <Alert className="bg-background/50 border-emerald-500/20 text-emerald-500">
                    ...
                    <AlertDescription className="text-xs text-muted-foreground mt-1">
                      ...
                      keys are correctly synchronized.
                    </AlertDescription>
                  </Alert>
                </div>
              </div>
          ...
          <CardHeader className="pb-3 border-b border-border/50">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4 bg-secondary/20 p-2 pr-4 rounded-full border border-border/50">
                <CircularProgress
                  value={secretsList.filter((s) => s.configured).length}
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {secretsList.filter((s) => s.configured).length} of{" "}
                    {secretsList.length} Set
                  </span>
                </div>
              </div>
            </div>
                ...
                <Button
                  ...
                  onClick={() => {
                    const cmds = secretsList
                      ...
                      .map(
                        (req) =>
                          `bun run scripts/manage.ts secrets update-cf ${req.secret} ${req.worker} "${req.example}"`
                      )
                      .join(" && ");
                    navigator.clipboard.writeText(cmds);
                    toast.success("All commands copied!", {
                      description:
                        "Paste into your terminal to configure all missing secrets.",
                    });
                  }}
                >
          ...
          <CardContent className="space-y-6 pt-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`font-medium text-sm ${req.configured ? "text-muted-foreground" : ""}`}
                              >
                                {req.secret}
                              </span>
                              {req.configured && (
            ...
            <Alert className="mt-6 bg-muted/50 border-border">
              ...
              <AlertDescription className="text-xs text-muted-foreground mt-1">
                ...
                Run the CLI command:{" "}
                <code className="text-[10px] font-[family-name:var(--font-geist-mono)] bg-background px-1 py-0.5 rounded border border-border">
