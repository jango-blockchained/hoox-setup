...
<Tip>Need to tail live worker logs to debug a custom strategy? Run `hoox logs tail trade-worker` in your terminal to open a real-time WebSocket connection to Cloudflare’s global edge logging console!</Tip>

### 🔗 Next Steps

- **[Debugging Runbook](../development/debugging)** — Diagnose local and remote V8 exceptions using diagnostic profiles.
- **[Self-Healing & Repair](../guides/repair)** — Recover from degraded workers and provision missing bindings.
