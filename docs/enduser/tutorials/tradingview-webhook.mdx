...
<Tip>Replace `"your-hoox-webhook-passkey"` in your Pine Script with the actual passkey configured in your `CONFIG_KV` store (`webhooks:api_key`). This is a critical security step—the gateway will reject any alerts with mismatched keys with a `401 Unauthorized` error.</Tip>
...
- **[Setting Up Telegram Alerts](telegram-bot)** — Integrate Telegram notifications to get fills pushed directly to your phone.
- **[API Endpoint Reference](../reference/api-endpoints)** — Review full request schemas and status codes.
