...
<ol>
  <li><strong>Passkey Verification</strong>: The gateway checks the <code>apiKey</code> property inside the JSON payload against your secure manifest in <code>CONFIG\_KV</code>.</li>
  <li><strong>Cloudflare WAF (Web Application Firewall)</strong>: Since TradingView publishes their official IP ranges, you can configure Cloudflare's edge firewall to block all webhook traffic that does not originate from these verified IPs.</li>
</ol>
