...
<ol>
  <li><strong>Telegram Webhook Registration</strong> — Tells Telegram's Bot API where to send user messages. Without this, your bot will be unreachable.</li>
  <li><strong>Internal URL Binding</strong> — After deployment, each worker gets a new URL. This command updates all Service Binding references so workers can find each other.</li>
  <li><strong>KV Manifest Sync</strong> — Ensures your runtime configuration (kill switch, rate limits, routing rules) matches the freshly deployed workers.</li>
</ol>
...
<ol>
  <li>The CLI compiles the Next.js app using the <strong>Turbopack</strong> build engine.</li>
  <li>The <strong>OpenNext</strong> compiler bundles the application logic into a single Edge-compliant V8 worker file: <code>.open-next/worker.js</code>.</li>
  <li>All static files (images, JS chunks, CSS) are isolated under <code>.open-next/assets/</code>.</li>
  <li>The CLI uses <code>wrangler</code> to deploy the worker and binds the static assets to Cloudflare's <strong>ASSETS binding</strong>, serving pages at sub-millisecond speeds.</li>
</ol>
