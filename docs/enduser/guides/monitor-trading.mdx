...
<Warning>When the Kill Switch is turned `ON`, the `hoox` gateway router immediately rejects all incoming TradingView alerts and API webhook requests with a `503 Service Unavailable` error and logs a `KILL_SWITCH_ACTIVE` warning. Active positions must be flattened manually or via `hoox monitor trades close-all`.</Warning>
...
<Tip>If the system status check returns a red error indicator (e.g. `d1-worker ... ❌ FAILED`), immediately proceed to the **[Self-Healing & Repair Guide](repair)** to run automated diagnostics and restore service bindings!</Tip>
