...
> **Heads up (v0.8.0):** The CLI surface was recently refactored. New top-level commands (`hoox onboard`, `hoox secrets`, `hoox keys`, `hoox perf`) were added; redundant commands (`hoox monitor status`, `hoox workers status`, `hoox dashboard update-urls`) were removed. See the [changelog](#v080-refactor) below.
