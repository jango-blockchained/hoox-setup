...
| **Leverage** | A multiplier applied to a position's notional value, allowing traders to control larger positions with less capital. E.g., 10x leverage on a $1,000 position controls $10,000. |
...
| **Side (BUY/SELL)** | The translated exchange-level direction. `LONG` → `BUY`, `SHORT` → `SELL`. |
...
| Term | Definition |
|------|-----------|
| **Iparamotency** | The property ensuring that a given operation (e.g., a trade signal) produces the same result whether executed once or multiple times — preventing duplicate trades. |
...
> **Keep this page bookmarked!** Cross-referenced from nearly every section of the documentation.