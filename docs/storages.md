...
- **Resource Constraints:** D1 has strict limits on the free tier (100k writes/day, 5M reads/day). Therefore, verbose or ephemeral data should *not* be stored here.
...
## 4. Storage Security
- All sensitive configuration keys (like API keys) are stored in the Cloudflare Secret Store, *not* in KV or D1.
