...
**Concept**: Run workers locally with `hoox dev start` (Native vs Docker runtime selection). The TUI (`packages/tui/`) provides a full-screen terminal dashboard for monitoring and management.
...
The Hoox TUI is a full-screen terminal dashboard for monitoring and managing workers, trades, logs, and configuration — all from your terminal.
...
# Launch directly (packages/tui/)
cd packages/tui && bun run dev
bun run start             # run built bundle
```

**Views (Ctrl+1-9):** Dashboard, Workers Overview, Worker Detail, Trade Monitor, Logs Viewer, Service Manager, Config Editor, Setup Wizard, Settings.  
**Shortcuts:** Ctrl+P (command palette), Ctrl+B (toggle sidebar), Ctrl+R (refresh), Ctrl+Q (quit).  
**Tech stack:** OpenTUI + React + Zustand, Bun runtime, TypeScript strict.  
**Source:** `packages/tui/`
