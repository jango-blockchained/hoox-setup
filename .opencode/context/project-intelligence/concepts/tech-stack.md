...
**Concept**: Bun monorepo with 10 Cloudflare Workers, D1, Smart Placement, Vectorize, Browser Rendering, Next.js 16 dashboard (Cloudflare Workers + OpenNext), an OpenTUI terminal dashboard (TUI), and Zod schema validation across the stack.
...
- **TUI** (`packages/tui/`): Full-screen terminal operations center built with OpenTUI (`@opentui/core` + `@opentui/react`), Zustand 5.x + Immer 10.x state management, Tree-sitter WASM syntax highlighting. Launched via `./hoox-tui`, `hoox tui`, or directly with `bun run packages/tui/src/main.tsx`.
...
- **Testing**: bun test (unit) — including TUI-specific tests, vitest + @cloudflare/vitest-pool-workers (integration)
...
| Astro              | 6 (docs site)                                 |
| OpenTUI            | @opentui/core@^0.1.0 + @opentui/react@^0.2.10 |
| Zustand            | 5.x (with Immer 10.x)                         |
...
**TUI context**: `.opencode/context/tui/` (standards.md, patterns.md, domain.md)
