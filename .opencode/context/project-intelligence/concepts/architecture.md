...
**Concept**: Hoox is a Cloudflare Edge Worker platform. `hoox` gateway routes external requests to internal workers via Service Bindings. 10 workers across the edge, all on Free plan. Dashboard uses Next.js 16 + OpenNext deployed to Cloudflare Workers. The **TUI** (`packages/tui/`) is a full-screen terminal dashboard built with OpenTUI for local monitoring and management. The `.opencode/` directory serves as the central project-knowledge hub (plans, specs, context, skills, tasks).
...
- **TUI** (`packages/tui/`): Full-screen terminal dashboard for monitoring and managing workers locally. Built with OpenTUI + React + Zustand. Launched via `./hoox-tui`, `hoox tui`, or `bun run packages/tui/src/main.tsx`.
...
**TUI**: `packages/tui/` — OpenTUI terminal dashboard
**TUI design spec**: `.opencode/specs/2026-05-14-tui-architecture-design.md`
**TUI context files**: `.opencode/context/tui/` (standards.md, patterns.md, domain.md)
**TUI tasks**: `.opencode/tasks/tui-dashboard/` (20 subtasks, completed)
