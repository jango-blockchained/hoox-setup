...
import { logKvTimestamp, headersToObject, kvTimestampMiddleware } from "../src/utils/kvUtils";
...
describe("KV Utilities", () => {
  ...
  test("headersToObject should handle errors during iteration", () => {
    ...
    expect(result).toEqual({});
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining("Error converting headers to object: Headers error")
    );
  });
