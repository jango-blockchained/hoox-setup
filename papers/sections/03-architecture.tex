...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llp{5.2cm}l@{}}
    ...
    \texttt{agent-worker} & Risk manager & Trailing stops, kill switch, LLM summary & Cron \texttt{*/5} \\
...
\begin{figure}[t]
  ...
  \IfFileExists{figures/architecture.pdf}{%
    ...
    \ifhooxusetikz
      \resizebox{0.98\linewidth}{!}{\input{figures/architecture.tex}}%
    \else
      \small
...
\begin{enumerate}
  ...
  \item \textbf{Risk management:} Every five minutes, \texttt{agent-worker} evaluates open positions and may adjust trailing stops or activate the global kill switch.
\end{enumerate}
...
Traditional microservices communicate over HTTP or gRPC, incurring serialization, TLS, and DNS costs at each hop. In \textsc{HOOX}, Service Bindings invoke internal endpoints as \texttt{binding.fetch("http://internal/path")} calls. Measured Service Binding overhead averages 0.4\,ms (Table~\ref{tab:production-latency}). Authorization is enforced on every binding call via an \texttt{X-Internal-Auth-Key} header validated by shared middleware.

Table~\ref{tab:bindings} summarizes the primary binding graph. The full code-graph community decomposition and data-flow edge catalog appear in Appendix~\ref{app:graph} (Figure~\ref{fig:graph-overview}).
