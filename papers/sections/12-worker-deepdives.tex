...
\paragraph{Cron sequence.} Figure~\ref{fig:agent-cron} shows the seven-step observation loop implemented in \texttt{processRoutine}.

\begin{figure}[!htbp]
  \centering
  \IfFileExists{figures/agent-cron-lifecycle.pdf}{%
    \includegraphics[width=\linewidth]{figures/agent-cron-lifecycle.pdf}%
  }{%
    \ifhooxusetikz
      \resizebox{\linewidth}{!}{\input{figures/agent-cron-lifecycle.tex}}%
    \else
      \fbox{\parbox{0.92\linewidth}{\small
      Cron $\rightarrow$ \texttt{agent-worker} $\rightarrow$ \texttt{d1-worker} / KV / exchanges $\rightarrow$ \texttt{trade-worker}\\
      \vspace{2pt}
      }}
    \fi
  }
  \caption{Five-minute agent cron risk loop (observation, KV watermarks, optional LLM summary, close dispatch).}
...
\begin{figure}[t]
  ...
  \IfFileExists{figures/report-cron-lifecycle.pdf}{%
    \includegraphics[width=\linewidth]{figures/report-cron-lifecycle.pdf}%
  }{%
    \ifhooxusetikz
      \resizebox{\linewidth}{!}{\input{figures/report-cron-lifecycle.tex}}%
    \else
