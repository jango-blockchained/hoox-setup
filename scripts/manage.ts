...
} from "child_process"; // Combined imports
...
async function loadConfig(): Promise<Config> {
  if (!fs.existsSync(CONFIG_PATH)) {
    ...
    console.error(
      yellow(
        "Please create a config.toml file in the project root or run a setup command that generates it."
      )
    );
    process.exit(1);
  }
  ...
  if (!parsedConfig.global) {
    ...
    console.warn(
      yellow(
        "Warning: [global] section missing in config.toml. Initializing empty."
      )
    );
  }
  if (!parsedConfig.workers) {
    ...
    console.warn(
      yellow(
        "Warning: [workers] section missing in config.toml. Initializing empty."
      )
    );
  }
  ...
  if (missingSettings.length > 0) {
    console.error(
      red(
        "Error: The following required global settings are still missing after prompt:"
      )
    );
  ...
  if (configWasModified) {
    ...
    const saveAnswer = await rl.question(
      blue(
        `Do you want to save these updated global settings to ${CONFIG_PATH}? (y/N): `
      )
    );
  ...
  [LOCAL_KEYS_FILE, PROD_KEYS_FILE].forEach((filePath) => {
    if (!fs.existsSync(filePath)) {
      ...
      try {
        ...
        console.warn(
          yellow(
            `Warning: Could not set permissions for ${path.basename(filePath)}. Error: ${err.message}`
          )
        );
      }
    }
...
function readKeys(
  ...
  try {
    ...
    console.error(
      red(`Error reading keys file (${environment}): ${error.message}`)
    );
  }
  return keys;
}
...
function writeKeys(
  ...
  try {
    ...
    console.error(
      red(`Error writing keys file (${environment}): ${error.message}`)
    );
  }
}
...
async function runCommandWithStdin(
  ...
  return new Promise((resolve) => {
    console.log(
      dim(`Running in ${cwd}: ${command} ${args.join(" ")} (with stdin)`)
    );
    ...
    child.on("error", (error) => {
      console.error(
        red(`Spawn error for command "${command}": ${error.message}`)
      );
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        console.error(
          red(
            `Error running command: ${command} ${args.join(" ")} (exit code: ${code})`
          )
        );
        console.error("Stderr:\n", stderr); // Log stderr on error
    ...
    try {
      ...
      console.error(
        red(`Error writing to stdin for command "${command}": ${error.message}`)
      );
...
async function getCloudflareToken(config: Config): Promise<string | null> {
  ...
  console.log(
    yellow(
      "Cloudflare API Token not found in environment variables or config.toml."
    )
  );
  const answer = await rl.question(
    blue("Please enter your Cloudflare API Token (or leave blank to skip): ")
  );
  ...
  console.error(
    red(
      "Cloudflare API Token is required for this operation and was not provided."
    )
  );
  return null;
}
...
async function runInteractiveCommand(
  ...
  // Returns exit code or null on error
  return new Promise((resolve) => {
    console.log(
      dim(`Running interactively in ${cwd}: ${command} ${args.join(" ")}`)
    );
    const fullEnv = { ...process.env, ...env };
    const child = spawn(command, args, {
      cwd: cwd,
    ...
    child.on("error", (error) => {
      console.error(
        red(
          `Spawn error for interactive command "${command}": ${error.message}`
        )
      );
    ...
    child.on("close", (code) => {
      console.log(
        dim(
          `Interactive command finished: ${command} ${args.join(" ")} (exit code: ${code})`
        )
      );
