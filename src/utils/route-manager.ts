...
export class RouteManager {
  ...
  async getRoutes(): Promise<RoutesConfig> {
    ...
    // Fetch from KV
    try {
      const routes = (await this.kv.get(
        ROUTES_CONFIG_KEY,
        "json"
      )) as RoutesConfig | null;

      if (!routes) {
        // No config found, use default and save it
        await this.kv.put(
          ROUTES_CONFIG_KEY,
          JSON.stringify(DEFAULT_ROUTES_CONFIG)
        );
  ...
  async findMatchingRoute(
    requestPath: string,
    method: string
  ): Promise<{ route: RouteConfig; params: Record<string, string> } | null> {
    const routes = await this.getRoutes();

    // First check for exact matches
    if (
      routes.routes[requestPath] &&
      routes.routes[requestPath].methods.includes(method)
    ) {
