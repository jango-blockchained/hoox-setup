...
describe('WorkerService', () => {
    ...
    it('should construct correctly', () => {
        const service = new WorkerService(mockSetWorkers, mockSetLogs, mockSetStatusMessage);
    ...
    describe('startWorker', () => {
        it('should spawn a worker process and update status', async () => {
            ...
            expect(mockSetStatusMessage).toHaveBeenCalledWith(`Starting ${workerId} worker...`);
            expect(mockSpawn).toHaveBeenCalledWith(
                'bun',
                ['run', 'dev', '--', '--port', '8787', '--local'], // Example args for d1
                expect.objectContaining({ cwd: expect.stringContaining(`${workerId}-worker`), shell: true })
            );
            ...
            expect(mockSetStatusMessage).toHaveBeenCalledWith(`${workerId} worker started on port 8787`);
         ...
         it('should handle spawn error', async () => {
            ...
            mockSpawn.mockImplementation(() => { throw new Error(errorMessage); });
            ...
            expect(mockSetStatusMessage).toHaveBeenCalledWith(`Error starting ${workerId} worker: ${errorMessage}`);
            expect(mockSetLogs).toHaveBeenCalledWith(expect.any(Function));
    ...
    describe('stopWorker', () => {
        it('should kill a running process and update status', async () => {
            ...
            expect(mockSetStatusMessage).toHaveBeenCalledWith(`Stopping ${workerId} worker...`);
            ...
            expect(mockSetStatusMessage).toHaveBeenCalledWith(`${workerId} worker stopped`);
            expect(service.workerProcesses[workerId]).toBeUndefined();
        });
...
`;
            ...
            expect(mockExec).toHaveBeenCalledWith("ps aux | grep 'bun run dev --' | grep -v grep");
            expect(mockSetWorkers).toHaveBeenCalledTimes(2); // Initial + Update

            // Check the final call to setWorkers to see the result
            const lastSetWorkersCallArg = mockSetWorkers.mock.calls[mockSetWorkers.mock.calls.length - 1][0];
